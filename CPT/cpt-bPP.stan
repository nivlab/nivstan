// Cumulative Prospect Theory - Partial Pooling - Version B
// 
// Parameters
// -- alpha:    risk aversion
...
transformed parameters {
    ...
    vector<lower=0,upper=2>[M] lambda;
    ...
    lambda  = Phi_approx( mu_pr[2] + sigma[2] * lambda_pr ) * 2;
    gamma   = Phi_approx( mu_pr[3] + sigma[3] * gamma_pr ) * 2;
    delta_p = Phi_approx( mu_pr[4] + sigma[4] * delta_p_pr ) * 2;
    delta_n = Phi_approx( mu_pr[5] + sigma[5] * delta_n_pr ) * 2;
    theta   = Phi_approx( mu_pr[6] + sigma[6] * theta_pr );
...
model { 
    ...
    for (i in 1:M) {
        ...
        // Gamble 1: value / probability
        vector[k] u1 = risk_sensitivity_neg( Xn[i,1,:k], alpha[sub_ix[i]], lambda[i] );
        vector[k] u2 = risk_sensitivity_neg( Xn[i,2,:k], alpha[sub_ix[i]], lambda[i] );
        vector[k] w1 = probability_weights( Pn[i,1,:k], gamma[i], delta_n[i] );
        
        // Gamble 2: value / probability
        vector[k] u3 = risk_sensitivity_neg( Xn[i,3,:k], alpha[sub_ix[i]], lambda[i] );
        vector[k] u4 = risk_sensitivity_neg( Xn[i,4,:k], alpha[sub_ix[i]], lambda[i] );
        vector[k] w2 = probability_weights( Pn[i,2,:k], gamma[i], delta_n[i] );
        
        // Compute difference in expected value
        vector[k] dEV;
        dEV = -fpow(fabs(u1 .* w1 + u2 .* (1-w1)) / lambda[i], 1/alpha[sub_ix[i]]) + 
               fpow(fabs(u3 .* w2 + u4 .* (1-w2)) / lambda[i], 1/alpha[sub_ix[i]]);
...
generated quantities {
    ...
    mu[5] = Phi_approx( mu_pr[5] ) * 2;
    mu[6] = Phi_approx( mu_pr[6] );
